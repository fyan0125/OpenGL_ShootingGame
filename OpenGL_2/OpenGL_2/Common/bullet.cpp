...
bullet::bullet(vec2 position)
{
	...
	CreateBufferObject();
	_position = position;
	mat4 mxGT = Translate(position[0],position[1], 0);
	setTRSMatrix(mxGT);
...
void bullet::setVtxColors(GLfloat vLFColor[], GLfloat vLRColor[], GLfloat vTRColor[], GLfloat vTLColor[])
{
...
void bullet::setPosition(vec2 newPosition)
{
...
void bullet::setVelocity(vec2 newVector)
{
	_velocity = newVector;
}
...
void bullet::update()
{
	...
	mat4 mxGT = Translate(_position[0], _position[1], 0);
	setTRSMatrix(mxGT);
}